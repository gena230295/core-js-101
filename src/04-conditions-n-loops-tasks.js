...
function getFactorial(n) {
  ...
  const newArr = arr.reduce((a, b) => a * b, 1);
  return newArr;
}
...
function isInsideCircle(circle, point) {
  ...
  const pointCircle = a + b;
  if (pointCircle < circle.radius ** 2) {
    return true;
  }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const str = `${isStartIncluded === false ? '(' : '['}${a < b ? a : b}, ${
    a > b ? a : b
  }${isEndIncluded === false ? ')' : ']'}`;
  return str;
}
...
function getDigitalRoot(num) {
  ...
  let newSum = 0;
  for (let i = 0; i < String(sum).length; i += 1) {
    newSum += getDigitalRoot(String(sum)[i]);
  }
  return newSum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let s = 0; s < sample.length; s += 1) {
      newStr = newStr.replace(sample[s], '');
    }
  }
  if (newStr.length === 0) {
    return true;
...
function getMatrixProduct(m1, m2) {
  ...
  let y = 0;
  let b = 0;
  for (let i = 0; i < m1.length * m2.length; i += 1) {
    let sum = 0;
  ...
  for (let i = 0; i < m1.length; i += 1) {
    finArr.push(arr.slice(i * m1.length, i * m1.length + m1.length));
  }
  return finArr;
}
